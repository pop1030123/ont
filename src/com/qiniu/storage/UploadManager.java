 ...
 * {@link com.qiniu.common.Config#PUT_THRESHOLD}
 */
public final class UploadManager {
	 ...
	 *            断点记录者
	 * @param keyGen
	 *            生成文件的断点记录标示，根据生成的标示，可找到断点记录的内容
	 */
	 ...
	 */
	public Response put(final byte[] data, final String key, final String token, StringMap params, String mime,
	 ...
	 *            是否验证crc32
	 */
	public Response put(String filePath, String key, String token, StringMap params, String mime, boolean checkCrc)
			throws QiniuException {
	...
	public Response put(File file, String key, String token, StringMap params, String mime, boolean checkCrc)
		...
		ResumeUploader uploader = new ResumeUploader(client, token, key, file, params, mime, recorder, recorderKey);
		return uploader.upload();
	}

	public Response put(InputStream stream, String key, String token, long fileSize, String filePath)
			throws QiniuException, IOException {
		return put(stream, key, token, null, null, false, fileSize, filePath);
	}

	public Response put(InputStream stream, String key, String token, StringMap params, String mime, boolean checkCrc,
			long fileSize, String filePath) throws QiniuException, IOException {
		if (mime == null) {
			mime = Client.DefaultMime;
		}
		params = filterParam(params);

		InputStreamChunkUploader uploader = new InputStreamChunkUploader(client, token, key, stream, params, mime, fileSize,
				filePath);

		return uploader.upload();
	}

	public void asyncPut(final byte[] data, final String key, final String token, StringMap params, String mime,
